...
if ($reserva_id) {
    try {
        $verifica = $pdo->prepare("SELECT * FROM reservas_restaurante WHERE id = :id AND id_usuario = :usuario_id");
        $verifica->execute([
            ':id' => $reserva_id,
        ...
        if ($verifica->rowCount() === 1) {
            $delete = $pdo->prepare("DELETE FROM reservas_restaurante WHERE id = :id");
            $delete->execute([':id' => $reserva_id]);

            $_SESSION['mensagem_sucesso'] = "Reserva do restaurante cancelada com sucesso.";
        } else {
